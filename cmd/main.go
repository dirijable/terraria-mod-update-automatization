...
func main() {
	...
	filteredDirs := mods.MustFilteredDirsFromPath(cleanedPath)
	localTmodPaths := mods.LatestTmodFiles(cleanedPath, filteredDirs)
	...
	cache, err := cache.LoadCacheFromFile(cacheFile)
	if err != nil {
	...
	if err := git.SyncRepo(*pathToRepo); err != nil {
		log.Fatalf("git sync failed: %v", err)
	}

	if err := mods.CopyUpdatedMods(filepath.Join(*pathToRepo, "Postavka"), modsToUpdate); err != nil {
		log.Fatalf("copy updated mods failed: %v", err)
	}

	if err := cache.SaveCacheToFile(cacheFile, cache); err != nil {
		log.Fatalf("failed to save cache: %v", err)
	}

	if err := git.PushUpdates(*pathToRepo, "auto-update tModLoader mods"); err != nil {
